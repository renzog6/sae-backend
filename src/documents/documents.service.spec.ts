...
describe('DocumentsService', () => {
  ...
  it('create calls prisma.document.create', async () => {
    ...
    const res = await service.create({ filename: 'f', mimetype: 'm', size: 1, path: 'p' });
    expect(prismaMock.document.create).toHaveBeenCalledWith({ data: { filename: 'f', mimetype: 'm', size: 1, path: 'p' } });
  ...
  it('findOne calls prisma.document.findUniqueOrThrow', async () => {
    ...
    expect(prismaMock.document.findUniqueOrThrow).toHaveBeenCalledWith({ where: { id: 1 } });
    expect(res).toEqual({ id: 1 });
  });

  it('update calls prisma.document.update', async () => {
    (prismaMock.document.update as any).mockResolvedValue({ id: 1, description: 'x' });
    const res = await service.update(1, { description: 'x' });
    expect(prismaMock.document.update).toHaveBeenCalledWith({ where: { id: 1 }, data: { description: 'x' } });
