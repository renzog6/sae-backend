...
describe('EmployeeVacationsService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [EmployeeVacationsService, { provide: PrismaService, useValue: prismaMock }],
    }).compile();
  ...
  it('create() should compute endDate and default year', async () => {
    ...
    (prisma.employeeVacation.create as any).mockImplementation(({ data }: any) => ({ id: 1, ...data }));
    ...
    expect(new Date(res.endDate).toISOString()).toBe(new Date('2025-01-19T00:00:00.000Z').toISOString());
  ...
  it('generateVacationPdf() should throw if template is missing', async () => {
    ...
    await expect(service.generateVacationPdf(1)).rejects.toBeInstanceOf(NotFoundException);
