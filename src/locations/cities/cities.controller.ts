// file: sae-backend/src/locations/cities/cities.controller.ts
...
export class CitiesController {
  ...
  byProvince(@Param('provinceId') provinceId: string) {
    return this.citiesService.findByProvince(+provinceId).then((data) => ({ data }));
  }
  ...
  byPostalCode(@Param('postalCode') postalCode: string) {
    return this.citiesService.findByPostalCode(postalCode).then((data) => ({ data }));
  }
}
