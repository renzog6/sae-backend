// file: sae-backend/src/locations/addresses/addresses.controller.ts
...
export class AddressesController {
  ...
  createForPerson(@Param('personId') personId: string, @Body() dto: CreateAddressDto) {
    return this.addressesService.createForPerson(+personId, dto).then((data) => ({ data }));
  }

  @Post('company/:companyId')
  @ApiOperation({ summary: 'Create address for a company' })
  @ApiParam({ name: 'companyId', description: 'Company ID' })
  createForCompany(@Param('companyId') companyId: string, @Body() dto: CreateAddressDto) {
    return this.addressesService.createForCompany(+companyId, dto).then((data) => ({ data }));
  }
