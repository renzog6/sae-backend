// filepath: sae-backend/src/modules/contacts/tests/contacts.service.spec.ts
...
describe("ContactsService", () => {
  ...
  describe("create", () => {
    it("creates a contact without links", async () => {
      (prismaMock.contact.create as any).mockResolvedValue({
        id: 1,
        type: "EMAIL",
        value: "a@b.com",
        contactLinks: [],
  ...
  describe("findByCompany", () => {
    it("filters contacts by company id", async () => {
      (prismaMock.contact.findMany as any).mockResolvedValue([{ id: 1 }]);
      (prismaMock.contact.count as any).mockResolvedValue(1);
      const res = await service.findByCompany("10", {
        page: 1,
        limit: 10,
        skip: 0,
      } as any);
  ...
  describe("update", () => {
    it("updates core fields and manages links", async () => {
      ...
      (prismaMock.contact.findUnique as any).mockResolvedValue({
        id: 1,
        contactLinks: [],
      });
  ...
  describe("remove", () => {
    it("deletes links then contact", async () => {
      ...
      expect(prismaMock.contactLink.deleteMany).toHaveBeenCalledWith({
        where: { contactId: 1 },
      });
