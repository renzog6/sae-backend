// filepath: sae-backend/src/contacts/contacts.controller.spec.ts
...
describe('ContactsController', () => {
  ...
  it('create delegates to service', async () => {
    ...
    const res = await controller.create({ type: 'EMAIL', value: 'a@b.com' } as any);
  ...
  it('findByCompany delegates to service', async () => {
    (serviceMock.findByCompany as any).mockResolvedValue({ data: [], meta: { total: 0, page: 1, limit: 10, totalPages: 0 } });
    const res = await controller.findByCompany('1', { page: 1, limit: 10 } as any);
    expect(serviceMock.findByCompany).toHaveBeenCalledWith('1', { page: 1, limit: 10 } as any);
    expect(res).toEqual({ data: [], meta: { total: 0, page: 1, limit: 10, totalPages: 0 } });
  });
  ...
  it('update delegates to service', async () => {
    ...
    expect(serviceMock.update).toHaveBeenCalledWith('1', { value: 'x@y.com' } as any);
    expect(res).toEqual({ data: { id: 1 } });
  });
